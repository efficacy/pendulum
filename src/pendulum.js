  ...
  return {
    ...
    link: function link(callback) {
      ...
      async.each(self.modules, function(module, done) {
        ...
        var child = exec('npm link').stderr.pipe(process.stderr);
        async.each(self.depends[module], function(dep, done) {
          fs.unlink(path.join(self.location[module], 'node_modules', dep));
          var child = exec('npm link ' + module).stderr.pipe(process.stderr);
        }, function(err) {
          done(err);
        });
      }, function(err) {
